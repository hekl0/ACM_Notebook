...
void Splay(Node *T) {
    while (true) {
        Node *p = T->p; if (!p) break;
        Node *pp = p->p;
        if (!pp) {  //Zig
            if (p->l == T) rightRotate(p); else leftRotate(p);
            break;
        }
        if (pp->l == p) {
            if (p->l == T) {  rightRotate(pp); rightRotate(p); } //ZigZig
            else {  leftRotate(p);rightRotate(pp); }//ZigZag
        } else {
            if (p->l == T) { rightRotate(p);leftRotate(pp); } //ZigZag
            else { leftRotate(pp);leftRotate(p); }//ZigZig
        }
    }
    root = T;
}
